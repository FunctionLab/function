    ...
    def check_fringe(self, slim_file, namespace=None):
        ...
        for tid in stids:
            ...
            if slim_term is None:
                logger.error('Slim term name does not exist (potentially obsolete term): %s', tid)
                continue
...
if __name__ == '__main__':
    ...
    parser.add_option("-u", dest="pub_filter", action="store_true", help="Filter annotations from high-throughput publications (>50 annotations)")
    ...
    if options.pub_filter and options.nspace is None:
        sys.stderr.write("--When filtering by publication, must provide GO namespace.\n")
        sys.exit()
