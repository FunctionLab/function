...
# open TAB, ">$otab";
# print TAB "GS.ID\tGS.Desc";

open HH, ">$otab";
print HH "net\tid\tterm\t\tsize\tnum.in\tsum.in\tavg.in\t";
print HH "num.out\tsum.out\tavg.out\t";
print HH "num.inout\tsum.inout\tavg.inout\t";
# print HH "Segrn\tAvg.Segrn\tLogPval\n";
print HH "segrn\tavg.segrn\n";


# (my $gmt_dir = $igmt) =~ s/\.gmt/_genesets/g; $gmt_dir =~ s/^.*\///g;
# my ($gs_desc, $gs_pid);
# if($igsfilt) {
#     if($igfilt) {
#         ($gs_desc, $gs_pid) = splitgmt($igmt, $igsfilt, $igfilt); }
#     else {
#         ($gs_desc, $gs_pid) = splitgmt($igmt, $igsfilt); } }
# else {
#     if($igfilt) {
#         ($gs_desc, $gs_pid) = splitgmt($igmt, $igfilt); }
#     else {
#         ($gs_desc, $gs_pid) = splitgmt($igmt); } }
...
my ($otmphub, $ohub, $num_in, $num_out, $num_inout, $sum_in, $sum_out, $sum_inout);
my ($net, $gs, $avg_in, $sd_in, $avg_out, $sd_out, $avg_inout, $segrn,
    $avg_segrn, $tstat, $df, $pval, $logp); my $ctxts;
# my %gs_net_seg = (); my %gs_net_tstat = (); my $count = 0;
my %gs_net_seg = (); my %gs_net_logp = (); my $count = 0;

print "\n";
foreach my $dab (@idab) {
    ($net = $dab) =~ s/\.dab$//g; $net =~ s/\.qdab$//g; $net =~ s/^.*\///g;
    if($infilt) { unless(exists $sel_net{$net}) { next; } }
    print "$net ...\n";
    
    ($ohub = $dab) =~ s/\.q*dab/\.hubber\.out/g; $ohub =~ s/^.*\///g;
    # ($otmphub = $dab) =~ s/\.q*dab/\.tmp\.hubout/g; $otmphub =~ s/^.*\///g;
    # ($ohub = $otmphub) =~ s/\.tmp//g;
    $count ++; $time = runtime(); print "$time:\t$count\t$ohub\n";

    if($ipairfilt) {
        $ctxts = join ' ', @{$sel_pairs{$net}};
        print "\t$ctxts\n";
    ...
    # open FH, "$otmphub" or die "Can't open $otmphub!";
    open FH, "$ohub" or die "Can't open $ohub!";
        chomp(my @f=<FH>); close FH; # `rm -f $otmphub`;
        # open HH, ">$ohub";

        # print HH "Net\tGS\tSize\tNum.In\tSum.In\tAvg.In\t";
        # print HH "Num.Out\tSum.Out\tAvg.Out\t";
        # print HH "Num.InOut\tSum.InOut\tAvg.InOut\t";
        # print HH "Segrn\tAvg.Segrn\tLogPval\n";
...
# $time = runtime(); print "\n$time: Printing matrix ...";

# print TAB "\n";

# foreach $gs (@gs_array) {
#     print TAB "$gs\t$gs_desc->{$gs}";
#     foreach $net (@idab) {
#         $net =~ s/\.dab$//g; $net =~ s/\.qdab$//g; $net =~ s/^.*\///g;

#         if($gs_net_seg{$gs}{$net} eq 'NA') { print TAB "\tNA"; next; }
#         print TAB "\t", sprintf("%.5g", $gs_net_seg{$gs}{$net});

        # if($gs_net_tstat{$gs}{$net} eq 'NA') { print TAB "\tNA"; next; }
        # print TAB "\t", sprintf("%.5g", $gs_net_tstat{$gs}{$net});
        
        # if($gs_net_logp{$gs}{$net} eq 'NA') { print TAB "\tNA"; next; }
        # print TAB "\t", sprintf("%.5g", $gs_net_logp{$gs}{$net});
#     }
#     print TAB "\n";
# }
# close TAB;
...
sub splitgmt {
    ...
    while(<GMT>) {
        ...
        # ($gs_desc{$q[0]} = $q[1]) =~ s/ \([0-9]*\)$//g;
        # ($pid = $q[0]) =~ s/ /_/g; $pid =~ s/:/_/g;
        (my $pid = $q[1]) =~ s/ \([0-9]*\)$//g;
        $pid = s/[ \/]/_/g; $pid =~ s/[;,'".]//g;
