...
# Output
open my $TAB, ">>$otab";
print $TAB "#ID\tMODE\tPAR\t#P\t#N\tAUC\tP10R\tP20R\tP50R\tAUPRC\tWAUPRC\n";
...
# Classification
my (@p, $nclass, $min_csize, %class_size, %dat_vec, $ngenes, %cv_idx, $y, $par);
my ($train_dat, $train_subdat, $test_dat, $test_subdat, $model, $A, $B);
my ($rclass, %orig_class, %tmp_score,
    %pred_score, $pred_out, $pred_tab, $eval_tab, $time);
...
foreach my $ssv (@idat) {
    ...
    if($ipar) {
        if ( $ipar =~ /^-?(?:\d+\.?|\.\d)\d*\z/ ) { $par = $ipar; }
        else { $par = $dat_par{$ssv}; } }
    else { $par = 1; }
    ...
    $nclass = scalar keys %class_size; # No. classes

    $min_csize = $ngenes; # Size of class w/ fewest examples
    foreach my $c (keys %class_size) {
        if($class_size{$c} < $min_csize) {
            $min_csize = $class_size{$c}; } }
    ...
    for(my $n=0; $n<$icvn; $n++) {
        ...
        foreach my $c (keys %class_size) {
            ...
            $cv_idx{$c} = get_cvcuts($dat_vec{$c}, $min_csize, $icvk);
        }
        ...
        for(my $k=0; $k<$icvk; $k++) {
            ...
            if(($imode eq 'mc1') or ($imode eq 'mc2')) {
                ...
                print_pred(\%pred_score, \%orig_class, $n, $pred_tab);
            }
            ...
            # MC - Crammer & Singer
                ...
                `cat tmp.pred >> $pred_tab; rm -f tmp.pred $model`;
            }
    ...
    ($eval_tab = $ssv) =~ s/_[a-z]*\.[a-z]*$/\.$imode\.eval\.txt/g;
    dcheck($pred_tab, $eval_tab, $par, $TAB);
...
sub print_pred {
    ...
    foreach my $idx (keys %$pscore) {
        ...
        print PL "$rep\t", $oc->{$idx}, "\t", sprintf("%.6f", $ps), "\n";
    }
    close PL;
}
...
sub dcheck {
    ...
    open PT, "$predt"; chomp(my @pred=<PT>); close PT;
    open ET, ">$evalt";

    print ET "#Label\tScore\tTP\tFP\tTN\tFN\tPR\tRC\n";

    my $P = my $N = my $idx = 0;
    my %pred_score = (); my %true_label = ();
    ...
    foreach (@pred) {
        ...
        $true_label{$p[0]}{$idx} = $p[1];
        $pred_score{$p[0]}{$idx} = $p[2];
        $idx++;
    }

    my $auc = my $auprc = my $wauprc = my $p10r = my $p20r = my $p50r = 0;
    for my $rep (sort keys %pred_score) {
        my $tp = my $fp = my $tn = my $fn = $idx = 0;
        my $in10 = my $in20 = my $in50 = 1;
        my $pr = my $rc = my $srr = 0;
        my $tauc = my $tauprc = my $twauprc = my $tp10r = my $tp20r = my $tp50r = 0;

        foreach (sort {$pred_score{$rep}{$b} <=> $pred_score{$rep}{$a}} keys %{$pred_score{$rep}}) {
            if($true_label{$rep}{$_} == 1) {
                $tp++; $rc = $tp / $P;

                if(($rc >= 0.10) and $in10) {
                    $tp10r = $tp / ($tp + $fp); $in10 = 0; }
                if(($rc >= 0.20) and $in20) {
                    $tp20r = $tp / ($tp + $fp); $in20 = 0; }
                if(($rc >= 0.50) and $in50) {
                    $tp50r = $tp / ($tp + $fp); $in50 = 0; }

                $srr += 1/($idx+1);
                $twauprc += $tp / ($tp + $fp) / ($idx + 1);
                $tauprc += $tp / ($tp + $fp); }
            else {
                $fp++;
                $tauc += $tp; }
            ...
            print ET "$rep\t$true_label{$rep}{$_}\t$pred_score{$rep}{$_}\t$tp\t$fp\t$tn\t$fn\t$pr\t$rc\n";
        }

        if(($tp == 0) or ($fp == 0)) {
            print "warning: Too few +ve true labels or -ve true labels\n";
            $tauc = $tauprc = $twauprc = 0;
        }
        else {
            $tauc = $tauc / $tp / $fp;
            $tauprc = $tauprc / $tp;
        ...
        $auc += $auc + ($tauc - $auc)/$rep;
        $p10r += $p10r + ($tp10r - $p10r)/$rep;
        $p20r += $p20r + ($tp20r - $p20r)/$rep;
        $p50r += $p50r + ($tp50r - $p50r)/$rep;
        $auprc += $auprc + ($tauprc - $auprc)/$rep;
        $wauprc += $wauprc + ($twauprc - $wauprc)/$rep;
    }

    $predt =~ s/\.$imode\.pred\.txt//g;
    print $FH "$predt\t$imode\t$ppar\t$P\t$N\t", sprintf("%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f",
...
Cost parameter for the chosen method. If this option is not provided, an optimal
parameter is chosen using cross-validation.
